...
class FileModel(QAbstractTableModel):
    ...
    def dropMimeData( self, mimedata, action, row, column, parentIndex ):
        return super(FileModel, self).dropMimeData(mimedata, action, row, column, parentIndex)

    def mimeTypes(self):
        return super(FileModel, self).mimeTypes()

    def mimeData(self, indexes):
        mimeData = super(FileModel, self).mimeData(indexes)
        mimeData.setUrls(["file:///%s" %(self._data[indexes[0].row()].filePath().replace("\\", "/"))])
        return mimeData
